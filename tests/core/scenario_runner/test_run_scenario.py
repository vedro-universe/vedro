...
@pytest.mark.asyncio
async def test_no_steps_passed(*, runner: MonotonicScenarioRunner, dispatcher_: Dispatcher):
    with given:
...
@pytest.mark.asyncio
async def test_scenario_skipped(*, runner: MonotonicScenarioRunner, dispatcher_: Dispatcher):
    with given:
        step_ = Mock()
...
@pytest.mark.asyncio
async def test_single_step_passed(*, runner: MonotonicScenarioRunner, dispatcher_: Dispatcher):
    with given:
        vstep = make_vstep()
...
@pytest.mark.asyncio
async def test_single_step_failed(*, runner: MonotonicScenarioRunner, dispatcher_: Dispatcher):
    with given:
...
@pytest.mark.asyncio
async def test_multiple_steps_passed(*, runner: MonotonicScenarioRunner, dispatcher_: Dispatcher):
    with given:
    ...
    with then("events fired"):
        ...
        step_result2 = scenario_result.step_results[1]
        assert dispatcher_.mock_calls == [
            call.fire(ScenarioRunEvent(scenario_result)),
...
@pytest.mark.asyncio
async def test_multiple_steps_failed(*, runner: MonotonicScenarioRunner, dispatcher_: Dispatcher):
    with given:
        exception = AssertionError()
